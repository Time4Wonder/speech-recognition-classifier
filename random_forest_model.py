...
print("training an Random Forest model")
rf_model = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1) # model initialization with 100 trees and parallel processing
rf_model.fit(data.X_train_sk, data.y_train_sk)

# Model Evaluation
y_pred_rf = rf_model.predict(data.X_test_sk)
print("Random Forest Classification Report:")
print(classification_report(data.y_test_sk, y_pred_rf, target_names=data.label_encoder_sk.classes_))
print("Random Forest Accuracy:", accuracy_score(data.y_test_sk, y_pred_rf))

# Custom predictions

def testing_model_on_data(my_audio_path):
    my_mffcs = cemfccs.extract_mfccs(my_audio_path, sr=16000, n_mfcc=13)
    custom_prediction = rf_model.predict([my_mffcs.mean(axis=1)])
